...
| 8 | Qt cannot open a window | `libxcb-cursor0` is not installed, so Qt 6.5+ aborts with "Could not load the Qt platform plugin xcb". All prior verification had been headless (`offscreen`), which never loads that plugin. Installing it needs root. | Extracted the library from the Ubuntu archive with `apt-get download` (no root needed) into `vendor/lib/`, launched with `LD_LIBRARY_PATH`. `vendor/` is gitignored. | Gets a window now without touching the system. The real fix is `sudo apt install libxcb-cursor0`, which only you can run; after that, `vendor/lib` can be deleted. |
| 9 | `just` is not installed | The justfile written during scaffolding is unusable as-is. | Left the justfile in place and documented the raw commands alongside every target in the README. | The targets are still the clearest documentation of how to run each piece, and `sudo apt install just` makes them work. |
